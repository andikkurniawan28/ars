...
<div class="container-fluid">
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('produk_rental.update', $produk_rental->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="jenis_konsol_id" id="jenis_konsol_id" class="form-control @error('jenis_konsol_id') is-invalid @enderror">
                        ...
                        @foreach($jenis_konsols as $jenis_konsol)
                            <option value="{{ $jenis_konsol->id }}" {{ $produk_rental->jenis_konsol_id == $jenis_konsol->id ? 'selected' : '' }}>
                                {{ $jenis_konsol->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama" id="nama" class="form-control @error('nama') is-invalid @enderror"
                           value="{{ old('nama', $produk_rental->nama) }}" required>
                    @error('nama')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="durasi" id="durasi" class="form-control @error('durasi') is-invalid @enderror"
                           value="{{ old('durasi', $produk_rental->durasi) }}" required>
                    @error('durasi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="harga" id="harga" class="form-control @error('harga') is-invalid @enderror"
                           value="{{ old('harga', $produk_rental->harga) }}" required>
                    @error('harga')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="poin" id="poin" class="form-control @error('poin') is-invalid @enderror"
                           value="{{ old('poin', $produk_rental->poin) }}" required>
                    @error('poin')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
